...
void initServer(void) {
    ...
    if (server.port != 0 &&
        listenToPort(server.port, server.ipfd, &server.ipfd_count) == CACHE_ERR)
        exit(1);
    ...
    if (server.aof_state == CACHE_AOF_ON) {
        ...
        if (server.aof_fd == -1) {
            cacheLog(CACHE_WARNING, "Can't open the append_only file: %s",
                     strerror(errno));
            exit(1);
        }
    }
...
void populateCommandTable(void) {
    int j;
    int numcommands = sizeof(cacheCommandTable) / sizeof(struct cacheCommand);
    for (j = 0; j < numcommands; j++) {
        struct cacheCommand *c = cacheCommandTable + j;
        char *f = c->sflags;
        int retval1, retval2;
        while (*f != '\0') {
            switch (*f) {
                case 'w':
                    c->flags |= CACHE_CMD_WRITE;
                    break;
                case 'r':
                    c->flags |= CACHE_CMD_READONLY;
                    break;
                case 'm':
                    c->flags |= CACHE_CMD_DENYOOM;
                    break;
                case 'a':
                    c->flags |= CACHE_CMD_ADMIN;
                    break;
                case 'p':
                    c->flags |= CACHE_CMD_PUBSUB;
                    break;
                case 's':
                    c->flags |= CACHE_CMD_NOSCRIPT;
                    break;
                case 'R':
                    c->flags |= CACHE_CMD_RANDOM;
                    break;
                case 'S':
                    c->flags |= CACHE_CMD_SORT_FOR_SCRIPT;
                    break;
                case 'l':
                    c->flags |= CACHE_CMD_SORT_LOADING;
                    break;
                case 't':
                    c->flags |= CACHE_CMD_STALE;
                    break;
                case 'M':
                    c->flags |= CACHE_CMD_SKIP_MONITOR;
                    break;
                case 'k':
                    c->flags |= CACHE_CMD_ASKING;
                    break;
                case 'F':
                    c->flags |= CACHE_CMD_FAST;
                    break;
                default:
                    cachePanic("Unsupported command flag");
                    break;
            }
            f++;
        }
        ...
        cacheAssert(retval1 == DICT_OK && retval2 == DICT_OK);
    }
}

void resetCommandTableStats(void) {
    int num_commands = sizeof(cacheCommandTable) / sizeof(struct cacheCommand);
    int j;
...
void cacheOpArrayInit(cacheOPArray *oa) {
    oa->ops = NULL;
    oa->num_ops = 0;
}

int cacheOpArrayAppend(cacheOPArray *oa, struct cacheCommand *cmd, int dbid,
                       cobj **argv, int argc, int target) {
    cacheOP *op;
    oa->ops = zre_alloc(oa->ops, sizeof(cacheOP) * (oa->num_ops + 1));
    op = oa->ops + oa->num_ops;
    op->cmd = cmd;
    op->db_id = dbid;
    op->argv = argv;
    op->argc = argc;
...
void cacheOpArrayFree(cacheOPArray *oa) {
    while (oa->num_ops) {
        int j;
        cacheOP *op;
        oa->num_ops--;
...
struct cacheCommand *lookupCmmandOrOriginal(Sds name) {
    struct cacheCommand *cmd = dictFetchValue(server.commands, name);
    if (!cmd) {
...
void propagate(struct cacheCommand *cmd, int db_id, cobj **argv, int argc,
               int flags) {
    if (server.aof_state != CACHE_AOF_OFF && flags & CACHE_PROPAGATE_AOF) {
        feedAppendOnlyFile(cmd, db_id, argv, argc);
    }
    if (flags & CACHE_PROPAGATE_REPL) {
        replicationFeedSlaves(server.slaves, db_id, argv, argc);
    }
}

void alsoPropagate(struct cacheCommand *cmd, int db_id, cobj **argv, int argc,
                   int target) {
    cacheOpArrayAppend(&server.also_propagate, cmd, db_id, argv, argc, target);
}

void forceCommandPropagation(cacheClient *c, int flags) {
    if (flags & CACHE_PROPAGATE_REPL) c->flags |= CACHE_FORCE_REPL;
    if (flags & CACHE_PROPAGATE_AOF) c->flags |= CACHE_FORCE_AOF;
}

void call(cacheClient *c, int flags) {
    long long dirty, start, duration;
    int client_old_flags = c->flags;
    if (server.loading && c->flags & CACHE_LUA_CLIENT) {
        flags &= ~(CACHE_CALL_SLOWLOG | CACHE_CALL_STATS);
    }
    if (c->flags & CACHE_LUA_CLIENT && server.lua_caller) {
        if (c->flags & CACHE_FORCE_REPL) {
            server.lua_caller->flags |= CACHE_FORCE_REPL;
        }
        if (c->flags & CACHE_FORCE_AOF) {
            server.lua_caller->flags |= CACHE_FORCE_AOF;
        }
    }
    if (flags & CACHE_CALL_SLOWLOG && c->cmd->proc != execCommand) {
        char *latency_event =
                (c->cmd->flags & CACHE_CMD_FAST) ? "fast-command" : "command";
        latencyAddSampleIfNeeded(latency_event, duration / 1000);
        slowlogPushEntryIfNeeded(c->argv, c->argc, duration);
    }

    if (flags & CACHE_CALL_STATS) {
        c->cmd->microseconds += duration;
        c->cmd->calls++;
    }
    if (flags & CACHE_CALL_PROPAGATE) {
        int flags = CACHE_PROPAGATE_NONE;
        if (c->flags & CACHE_FORCE_REPL) flags |= CACHE_PROPAGATE_REPL;
        if (c->flags & CACHE_FORCE_AOF) flags |= CACHE_FORCE_AOF;
        if (dirty) flags |= (CACHE_PROPAGATE_REPL | CACHE_PROPAGATE_AOF);
        if (flags != CACHE_PROPAGATE_NONE)
            propagate(c->cmd, c->db->id, c->argv, c->argc, flags);
    }
    c->flags &= ~(CACHE_FORCE_AOF | CACHE_FORCE_REPL);
    c->flags |= client_old_flags & (CACHE_FORCE_AOF | CACHE_FORCE_REPL);
    if (server.also_propagate.num_ops) {
        int j;
        cacheOP *rop;
        for (j = 0; j < server.also_propagate.num_ops; j++) {
            rop = &server.also_propagate.ops[j];
            propagate(rop->cmd, rop->db_id, rop->argv, rop->argc, rop->target);
        }
        cacheOpArrayFree(&server.also_propagate);
    }
...
int processCommand(cacheClient *c) {
    if (!strcasecmp(c->argv[0]->ptr, "quit")) {
        ...
        c->flags |= CACHE_CLOSE_AFTER_REPLY;
        return CACHE_ERR;
    }
    ...
    if (server.cluster_enabled && !(c->flags & CACHE_MASTER) &&
        !(c->flags & CACHE_LUA_CLIENT && server.lua_caller->flags & CACHE_MASTER) &&
        !(c->cmd->getkeys_proc == NULL && c->cmd->firstkey == 0)) {
        int hashslot;
        if (server.cluster->state != CACHE_CLUSTER_OK) {
            flagTransaction(c);
            clusterRedirectClient(c, NULL, 0, CACHE_CLUSTER_REDIR_DOWN_STATE);
            return CACHE_OK;
        } else {
            int error_code;
    ...
    if (server.maxmemory) {
        ...
        if ((c->cmd->flags & CACHE_CMD_DENYOOM) && retval == CACHE_ERR) {
            flagTransaction(c);
            addReply(c, shared.oomerr);
            return CACHE_OK;
        }
    }
    if (((server.stop_writes_on_bgsave_err && server.saveparamslen > 0 && server.lastbgsave_status == CACHE_ERR) ||
         server.aof_last_write_status == CACHE_ERR) && server.masterhost == NULL &&
        (c->cmd->flags & CACHE_CMD_WRITE || c->cmd->proc == pingCommand)) {
        flagTransaction(c);
        if (server.aof_last_write_status == CACHE_OK)
            addReply(c, shared.bgsaveerr);
        else
            addReplySds(c, sdsCatPrintf(sdsEmpty(), "-MISCONF Errors waiting to the AOF file: %s\r\n",
                                        strerror(server.aof_last_write_errno)));
        return CACHE_OK;
    }
    if (server.masterhost == NULL && server.repl_min_slaves_to_write && server.repl_min_slaves_max_lag &&
        c->cmd->flags & CACHE_CMD_WRITE && server.repl_good_slaves_count < server.repl_min_slaves_to_write) {
        flagTransaction(c);
        addReply(c, shared.noreplicaserr);
        return CACHE_OK;
    }
    if (server.masterhost && server.repl_slave_ro && !(c->flags & CACHE_MASTER) && c->cmd->flags & CACHE_CMD_WRITE) {
        addReply(c, shared.roslaveerr);
        return CACHE_OK;
    }
    if (c->flags & CACHE_PUBSUB && c->cmd->proc != pingCommand && c->cmd->proc != subscribeCommand &&
        c->cmd->proc != unsubscribeCommand && c->cmd->proc != psubscribeCommand &&
        c->cmd->proc != punsubscribeCommand) {
        addReplyError(c, "only (P)SUBSCRIBE / (P)UNSUBSCRIBE QUIT allowed in this context");
        return CACHE_OK;
    }
    if (server.masterhost && server.repl_state != CACHE_REPL_CONNECTED && server.repl_serve_stale_data == 0 &&
        !(c->cmd->flags & CACHE_CMD_STALE)) {
        flagTransaction(c);
        addReply(c, shared.masterdownerr);
        return CACHE_OK;
    }

    if (server.loading && !(c->cmd->flags & CACHE_CMD_LOADING)) {
        addReply(c, shared.loadingerr);
        return CACHE_OK;
    }
    if (server.lua_timeout && c->cmd->proc != authCommand && c->cmd->proc != replconfCommand &&
        !(c->cmd->proc == shutdownCommand && c->argc == 2 &&
          tolower(((char *) c->argv[1]->ptr)[0]) == '\n') &&
        !(c->cmd->proc == scriptCommand && c->argc == 2 && tolower(((char *) c->argv[1]->ptr)[0]) == 'k')) {
        flagTransaction(c);
