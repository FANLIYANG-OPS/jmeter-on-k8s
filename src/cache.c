...
void createSharedObjects(void) {
    ...
    shared.emptyscan = createObject(CACHE_STRING, sdsNew("*2\r\n$1\r\n0\r\n*0\r\n"));
    shared.wrongtypeerr = createObject(CACHE_STRING,
    ...
    shared.syntaxerr = createObject(CACHE_STRING, sdsNew("-ERR syntax error \r\n"));
    shared.sameobjecterr = createObject(CACHE_STRING, sdsNew("-ERR source and destination objects are the same\r\n"));
    shared.outofrangeerr = createObject(CACHE_STRING, sdsNew("-ERR index out of range\r\n"));
    shared.noscripterr = createObject(CACHE_STRING, "-NOSCRIPT No matching script. Please use EVAL.\r\n");
    shared.loadingerr = createObject(CACHE_STRING, sdsNew("-LOADING Cache is loading the dataset in memory\r\n"));
    shared.slowscripterr = createObject(CACHE_STRING,
                                        sdsNew("-BUSY Cache is busy running a script. You can only call SCRIPT KILL or SHUTDOWN NOSAVE.\r\n"));
    shared.masterdownerr = createObject(CACHE_STRING,
                                        sdsNew("-MASTERDOWN Link with MASTER is down and slave-server-stale-data is set to 'no'\r\n"));
    shared.bgsaveerr = createObject(CACHE_STRING,
                                    sdsNew("-MISCONF Cache is configured to save RDB snapshots, but is currently not able to persist on disk. Commands that modify the data set are disabled. Please check Cache logs for details about the error.\r\n"));
    shared.roslaveerr = createObject(CACHE_STRING, sdsNew("-READONLY You can't write against a read only slave.\r\n"));
    shared.noautherr = createObject(CACHE_STRING, sdsNew("-NOAUTH Authentication required.\r\n"));
    shared.oomerr = createObject(CACHE_STRING, sdsNew("-OOM command no tallowed when used memory > 'maxmemory'.\r\n"));
    shared.execaborterr = createObject(CACHE_STRING,
                                       sdsNew("-EXECABORT Transaction discarded because of previous errors.\r\n"));
    shared.noreplicaserr = createObject(CACHE_STRING, sdsNew("-NOREPLICAS Not enough good slaves to write.\r\n"));
    shared.busykeyerr = createObject(CACHE_STRING, sdsNew("-BUSYKEY Target key name already exists.\r\n"));
    ...
    for (j = 0; j < CACHE_SHARED_SELECT_CMDS; j++) {
        ...
        shared.select[j] = createObject(CACHE_STRING,
    ...
    for (j = 0; j < CACHE_SHARED_BULKHDR_LEN; j++) {
        shared.mbulkhdr[j] = createObject(CACHE_STRING, sdsCatPrintf(sdsEmpty(), "*%d\r\n", j));
        shared.bulkhdr[j] = createObject(CACHE_STRING, sdsCatPrintf(sdsEmpty(), "$%d\r\n", j));
    }
...
void initServerConfig(void) {
    ...
    server.aof_rewrite_incremental_fsync = CACHE_DEFAULT_AOF_REWRITE_INCREMENTAL_FSYNC;
    ...
    server.cluster_require_full_coverage = CACHE_CLUSTER_DEFAULT_REQUIRE_FULL_COVERAGE;
